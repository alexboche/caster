# this file contains commands for making dragonfly commands faster
...
class DragonflyDevCommandsRule(MergeRule):
    mapping = {
        ...
        # eventually would be cool if we had one big toml file for storing mouse positions in different applications.
        # then there would be a command that automatically records 1) the mouse position, the active application, and a keyword for calling it. 
        # the command would automatically take all that information and place it inside the toml file so that so that the mouse position 
        # could then be called by the keyword while in that application active inputs it into the toml file
        # this would supplement the sikuli commands. thanks to lunis for this idea
