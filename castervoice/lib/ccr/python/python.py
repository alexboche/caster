...
class PythonNon(MappingRule):
    mapping = {
        ...
        "open file": R(
            Text("open('filename','r') as f:"), rdescript="Python: Open File"
        ),
        "read lines": R(
            Text("content = f.readlines()"), rdescript="Python: Read Lines"
        ),
        "try catch": R(
            Text("try:")
...
class Python(MergeRule):
    ...
    mapping = {
        ...
        #
        SymbolSpecs.FOR_EACH_LOOP: R(
            Text("for  in :") + Key("left:5"), rdescript="Python: For Each Loop"
        ),
        SymbolSpecs.FOR_LOOP: R(
            Text("for i in range(0, ):") + Key("left:2"), rdescript="Python: For i Loop"
        ),
        SymbolSpecs.WHILE_LOOP: R(
            Text("while :") + Key("left"), rdescript="Python: While"
        ),
        # (no do-while in Python)
        #
        SymbolSpecs.TO_INTEGER: R(
            Text("int()") + Key("left"), rdescript="Python: Convert To Integer"
        ),
        SymbolSpecs.TO_FLOAT: R(
            Text("float()") + Key("left"), rdescript="Python: Convert To Floating-Point"
        ),
        SymbolSpecs.TO_STRING: R(
            Text("str()") + Key("left"), rdescript="Python: Convert To String"
        ),
        #
        ...
        #
        SymbolSpecs.FUNCTION: R(
            Text("def ():") + Key("left:3"), rdescript="Python: Function"
        ),
        SymbolSpecs.CLASS: R(Text("class :") + Key("left"), rdescript="Python: Class"),
        #
        SymbolSpecs.COMMENT: R(Text("#"), rdescript="Python: Add Comment"),
        SymbolSpecs.LONG_COMMENT: R(
            Text("''''''") + Key("left:3"), rdescript="Python: Long Comment"
        ),
        #
        ...
        "it are in": R(
            Text(" in "), rdescript="Python: In"
        ),  # supposed to sound like "iter in"
        "shell iffae | LFA": R(
            Key("e,l,i,f,space,colon,left"), rdescript="Python: Else If"
        ),
        "convert to character": R(
            Text("chr()") + Key("left"), rdescript="Python: Convert To Character"
        ),
        ...
        "list (comprehension | comp)": R(
            Text("[x for x in TOKEN if TOKEN]"), rdescript="Python: List Comprehension"
        ),
        "[dot] (pie | pi)": R(Text(".py"), rdescript="Python: .py"),
        "toml": R(Text("toml"), rdescript="Python: toml"),
        "jason": R(Text("toml"), rdescript="Python: json"),
        "identity is": R(Text(" is "), rdescript="Python: is"),
        #"yield": R(Text("yield "), rdescript="Python: Yield"),
        ...
        "try [<exception>] as": R(
            ...
            + Pause("10")
            + Key("enter/2")
            + Text("except %(exception)s as :")
            + Pause("10")
            + Key("enter/2"),
            rdescript="create 'try catch as' block with given exception",
        ),
        ...
        "meth [<binary_meth>]": R(
            Text("def __%(binary_meth)s__(self, other):") + Key("enter"),
            rdescript="Python: binary special method",
        ),
        "meth [<unary_meth>]": R(
            Text("def __%(unary_meth)s__(self):") + Key("enter"),
            rdescript="Python: unary special method",
        ),

        "word <python_keyword>": R(
            Text(" %(python_keyword)s "), rdescript="Python: keyword"),
        "builtin <python_builtin>": R(
            Text(" %(python_builtin)s "), rdescript="Python: builtin"),
        "lib <python_standard_library>": 
            R(Text(" %(python_standard_library)s ")),
        "import <python_standard_library>": 
            R(Text("import %(python_standard_library)s ")),
